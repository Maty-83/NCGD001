...
public class StormTrapController : MonoBehaviour
{
    [Header("References")]
    ...
    public float LightningMaxInterval = 2f;
    public float LightningMinInterval = 1f;
    ...
    private float intervalSinceLastLightning = 0f;
    ...
    private void FixedUpdate()
    {
        ...
        intervalSinceLastLightning += Time.fixedDeltaTime;
    }

    void CalculateSpawnBounds()
    {
        Vector3 pos = BackgroundLayer.transform.position;

        spawnMinX = pos.x - BackgroundWidth / 3.5f;
        spawnMaxX = pos.x + BackgroundWidth / 3.5f;
        spawnY = pos.y;
    }
    ...
    void SpawnMultipleLeaves()
    {
        ...
        for (int i = 0; i < leafCount; i++)
        {
            ...
            var prefab = RainDropPrefab;
            if (Random.Range(0f, LightningMaxInterval) > LightningMaxInterval - intervalSinceLastLightning 
                && LightningMinInterval < intervalSinceLastLightning)
            {
                if (LightningPrefab != null)
                {
                    prefab = LightningPrefab;
                    intervalSinceLastLightning = 0;
                }
            }
            else
            {
                spawnPos.y += BackgroundHeight;
            }

            GameObject newLeaf = Instantiate(prefab, spawnPos, Quaternion.identity);

            var lightningTrack = newLeaf.GetComponent<LightningController>();
            if (lightningTrack != null)
            {
                lightningTrack.Init(AudioSource);
            }

            var windTrack = newLeaf.GetComponent<WaterDropController>();
            if (windTrack != null)
            {
                windTrack.SetController(this);
            }
